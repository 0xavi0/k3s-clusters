...
STOP_AFTER_LONGHORN_INSTALL = ((ENV["STOP_AFTER_LONGHORN_INSTALL"] || "no") == "yes")
S3GW_IMAGE                  = ENV["S3GW_IMAGE"] || "ghcr.io/aquarist-labs/s3gw:latest"
S3GW_IMAGE_PULL_POLICY      = ENV["S3GW_IMAGE_PULL_POLICY"] || "Always"
S3GW_UI_IMAGE               = ENV["S3GW_UI_IMAGE"] || "ghcr.io/aquarist-labs/s3gw-ui:latest"
S3GW_UI_IMAGE_PULL_POLICY   = ENV["S3GW_UI_IMAGE_PULL_POLICY"] || "Always"
...
CLUSTER_NAME                = ENV["CLUSTER_NAME"] || "cluster1"
ADMIN_NODE_PREFIX           = "#{CLUSTER_NAME}-admin"
WORKER_NODE_PREFIX          = "#{CLUSTER_NAME}-worker"

ansible_groups = {
  "apt" => [],
  "zypper" => [],
  "master" => [
    "#{ADMIN_NODE_PREFIX}-[1:#{ADMIN_COUNT}]"
  ],
  "node" => [
    "#{WORKER_NODE_PREFIX}-[1:#{WORKER_COUNT}]"
  ],
  "k3s_cluster" => [
    "#{ADMIN_NODE_PREFIX}-[1:#{ADMIN_COUNT}]",
    "#{WORKER_NODE_PREFIX}-[1:#{WORKER_COUNT}]"
...
extra_vars = {
  ...
  extra_server_args: "--node-ip #{VM_NET}.#{VM_NET_LAST_OCTET_START} --bind-address #{VM_NET}.#{VM_NET_LAST_OCTET_START} --flannel-iface=eth1",
...
extra_vars_vbox = {
  ...
  systemd_dir: "/etc/systemd/system",
  master_ip: "#{VM_NET}.#{VM_NET_LAST_OCTET_START}",
  master_name: "#{ADMIN_NODE_PREFIX}-1",
...
extra_provision_vars = extra_vars
if VM_PROVIDER == "virtualbox"
  extra_provision_vars = extra_vars_vbox
end
...
def ansible_provision (context, ansible_groups, extra_vars)
  ...
  if(!STOP_AFTER_BOOTSTRAP)
    ...
    if(!STOP_AFTER_K3S_INSTALL)
      ...
      if(!STOP_AFTER_LONGHORN_INSTALL)
        context.vm.provision "ansible" do |ansible|
          ansible.limit = "all"
...
Vagrant.configure("2") do |config|
  ...
  if BOX_NAME.include? "generic/ubuntu"
    ansible_groups["apt"] << "#{ADMIN_NODE_PREFIX}-[1:#{ADMIN_COUNT}]"
    ansible_groups["apt"] << "#{WORKER_NODE_PREFIX}-[1:#{WORKER_COUNT}]"
  elsif BOX_NAME.include? "opensuse/"
    ansible_groups["zypper"] << "#{ADMIN_NODE_PREFIX}-[1:#{ADMIN_COUNT}]"
    ansible_groups["zypper"] << "#{WORKER_NODE_PREFIX}-[1:#{WORKER_COUNT}]"
  end
  ...
  (1..ADMIN_COUNT).each do |i|
      config.vm.define "#{ADMIN_NODE_PREFIX}-#{i}" do |admin|
        ...
        admin.vm.hostname = "#{ADMIN_NODE_PREFIX}-#{i}"
		...
		if WORKER_COUNT == 0
      ansible_provision config, ansible_groups, extra_provision_vars
		end
  end

	if WORKER_COUNT > 0
		(1..WORKER_COUNT).each do |i|
			config.vm.define "#{WORKER_NODE_PREFIX}-#{i}" do |worker|
				...
				worker.vm.hostname = "#{WORKER_NODE_PREFIX}-#{i}"
        if VM_PROVIDER == "libvirt"
          ...
          worker.vm.network "public_network", bridge: VM_BRIDGE_INET, ip: "#{VM_NET}.#{VM_NET_LAST_OCTET_START+i+(ADMIN_COUNT-1)}"
				...
				if i == WORKER_COUNT
          ansible_provision worker, ansible_groups, extra_provision_vars
				end
			end
		end
	end
