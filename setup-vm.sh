...
export VM_PROVIDER=${VM_PROVIDER:-"virtualbox"}
export VM_NET=${VM_NET:-"192.168.1.0"}
export VM_NET_LAST_OCTET_START=${CLUSTER_NET_LAST_OCTET_START:-"51"}
export VM_BRIDGE_INET=${VM_BRIDGE_INET:-"en0"}
...
export ADMIN_DISK_SIZE=${ADMIN_DISK_SIZE:-"4G"}
export WORKER_MEM=${WORKER_MEM:-"4096"}
export WORKER_CPU=${WORKER_CPU:-"2"}
export WORKER_DISK=${WORKER_DISK:-"no"}
export WORKER_DISK_SIZE=${WORKER_DISK_SIZE:-"4G"}
export CONTAINER_ENGINE=${CONTAINER_ENGINE:-"podman"}
export STOP_AFTER_BOOTSTRAP=${STOP_AFTER_BOOTSTRAP:-"no"}
export STOP_AFTER_K3S_INSTALL=${STOP_AFTER_K3S_INSTALL:-"yes"}
export STOP_AFTER_LONGHORN_INSTALL=${STOP_AFTER_LONGHORN_INSTALL:-"yes"}
...
export S3GW_UI_IMAGE=${S3GW_UI_IMAGE:-"ghcr.io/aquarist-labs/s3gw-ui:latest"}
export S3GW_UI_IMAGE_PULL_POLICY=${S3GW_UI_IMAGE_PULL_POLICY:-"Always"}
export PROV_USER=${PROV_USER:-"vagrant"}

#these defaults will change
export S3GW_UI_REPO=${S3GW_UI_REPO:-"https://github.com/aquarist-labs/s3gw-ui.git"}
export S3GW_UI_VERSION=${S3GW_UI_VERSION:-"main"}
...
help() {
  echo "usage: $0 -c cluster id [OPTION]"
  echo "Where OPTION is:"
  echo "  build: Builds a k3s cluster"
  echo "  start: Starts a k3s cluster"
  echo "  destroy: Destroys a k3s cluster"
  exit 0
}
...
if [[ "$CLUSTER_ID" == "" ]]; then
   echo "-c parameter is mandatory"
   echo ""
   help
fi

if [[ "$LAST_IP_OCTET_START" == "" ]]; then
   echo "-o parameter is mandatory"
   echo ""
   help
fi

export VM_NET_LAST_OCTET_START=$LAST_IP_OCTET_START
